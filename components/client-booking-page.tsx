...
export function ClientBookingPage({ settings, services, barbers }: Props) {
  ...
  const totalDuration = cart.reduce((sum, item) => sum + (item.service.duration_minutes * item.quantity), 0)

  useEffect(() => {
    if (selectedBarber) {
      loadBarberSchedules(selectedBarber.id)
    }
  }, [selectedBarber])
  ...
  async function handleSubmit() {
    ...
    if (!selectedBarber || !selectedDate || !selectedTime || cart.length === 0) {
  ...
  function isDateAvailable(date: Date): boolean {
    ...
    return barberSchedules.some(s => s.day_of_week === dayOfWeek && s.is_working)
  }
                ...
                {next7Days.map((date) => {
                  ...
                  return (
                    <Card 
                      ...
                      className={`min-w-[100px] cursor-pointer transition-all ${
                        !available 
      ...
      {step === 'services' && cart.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-card border-t border-border">
          ...
          >
            ...
            Continuar - {formatCurrency(cartTotal)}
          </Button>
        </div>
      )}
